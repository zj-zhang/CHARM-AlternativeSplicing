# define the functions for heatmap plotting
...
plot_heatmap = function(plot_data, clab, main="heatmap.3", do_scale=T) {
    ...
    this_clab = as.matrix(clab[match_index,])
    has_var_row = which(apply(y, 1, var)>0)
    print(length(has_var_row))
    y = y[has_var_row, ]

    hr <- hclust(as.dist(1-cor(t(y), method="spearman", use="complete")), method="average")
    hc <- hclust(as.dist(1-cor(y, method="spearman", use="complete")), method="average")
